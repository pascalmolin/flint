...
struct mf_eis_dec {
    ...
    const long denom;
    const long modp;
    const long * coefs; /* nchi * degy values mod modp */
};

const struct mf_eis_dec f11 = { // [ -3/2, 5/2 ]
    11, 1, (const long[]){ 10 }, 1, 2, 1, (const long[]){ -3, 5 }
};
const struct mf_eis_dec f31 = { // [3/2*y - 7/2, 1/2*y + 1/6, -y + 4/3]
    31, 2, (const long[]){ 30,6 }, 2, 6, 1, (const long[]){ -21, 9, 1, 3, 8, -6 }
};
const struct mf_eis_dec f61 = {
